...
void Shapes::add(size_t index, direct_shape* S) {
...
direct_shape* Shapes::operator[](size_t index) {
	if (index >= size || index < 0) {
		cout << "Invalid index!" << endl;
		exit(-1);
	}
...
direct_shape* Shapes::copy(direct_shape* v) const {
	if (v == nullptr) {
		return nullptr;
	}

	direct_shape* copiedShape = nullptr;
